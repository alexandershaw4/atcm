function pong_tcm_active_inference_intercept_v2_demo
% PONG_TCM_ACTIVE_INFERENCE_INTERCEPT_V2_DEMO
%
% Updated drop-in replacement for the teacher-dependent Pong-TCM demo.
%
% Core change:
...
ctrl.k_intercept        = 8.0;      % steeper target, stronger directional signal      % slope of target tanh for intercept error
...
ctrl.eta_W              = 5e-3;     % faster normalized LMS learning     % normalized LMS learning rate
...
% Curriculum / scaffold parameters.
% The self-supervised intercept target initially helps drive action selection,
% then fades out so the learned TCM readout increasingly controls behaviour.
ctrl.curriculum_steps   = 2500;     % larger = slower fade from target to TCM
ctrl.min_scaffold       = 0.0;      % set e.g. 0.05 for a persistent weak scaffold
...
traj.v_control     = zeros(1, T_steps);
...
for t = 1:T_steps
    ...
    % Recompute readout after the update.
    v_pred = tanh(W_motor * x_tcm);

    % 7) Curriculum control signal.
    %    Early on, action selection is scaffolded by the self-supervised
    %    intercept target. The scaffold fades away, leaving the TCM readout
    %    to control behaviour. This is not the old dy teacher; it is a
    %    physics-derived target from the predicted future crossing point.
    alpha = max(ctrl.min_scaffold, 1 - t / ctrl.curriculum_steps);
    v_control = alpha * v_target + (1 - alpha) * v_pred;

    % 8) Select next action using short-horizon model-based rollout.
    a_t = select_action_pong_intercept_rollout(x_env, v_control, prev_action, env, dt_env, actions, plan);
    prev_action = a_t;

    % 9) Live dashboard.
    if mod(t, ctrl.plot_every) == 0
        draw_pong_frame_intercept(x_env, env, v_pred, v_target, v_control, alpha, a_t, intercept, event, t*dt_env);
    end
...
%
% V2 puts the intercept-relevant information first, so even legacy TCMs with
% M.ninputs = 3 receive the most useful quantities.
%
% Components:
%   1 dy_intercept    predicted future ball-paddle error at paddle crossing
%   2 dy_now          current ball-paddle vertical error
%   3 ball_vy         current ball vertical velocity
%   4 ball_x          current ball x
%   5 ball_vx         current ball horizontal velocity
...
s_t = [intercept.dy_intercept; dy_now; vy; bx; vx; double(intercept.approaching)];
end
...
% Plotting / animation
% =========================================================================
function draw_pong_frame_intercept(x_env, env, v_pred, v_target, v_control, alpha, a_t, intercept, event, t)
...
if isempty(hFig) || ~isvalid(hFig)
    ...
    hAx.control_line = plot(nan, nan, 'b--', 'LineWidth',0.9);
    ylabel('drive'); legend({'intercept target','TCM','control'}, 'Location','eastoutside');
    title('Target, TCM readout and fading control scaffold');
...
% Target vs readout vs actual control signal.
x1 = get(hAx.target_line,'XData');  y1 = get(hAx.target_line,'YData');
x2 = get(hAx.pred_line,'XData');    y2 = get(hAx.pred_line,'YData');
x3 = get(hAx.control_line,'XData'); y3 = get(hAx.control_line,'YData');
set(hAx.target_line, 'XData',[x1 t], 'YData',real([y1 v_target]));
set(hAx.pred_line,   'XData',[x2 t], 'YData',real([y2 v_pred]));
set(hAx.control_line,'XData',[x3 t], 'YData',real([y3 v_control]));
subplot(hAx.target);
title(sprintf('Target, TCM readout and control scaffold (alpha=%.2f)', alpha));
...
plot(t, traj.v_control, '--', 'LineWidth',1.0);
ylabel('drive'); legend({'intercept target','TCM readout','control signal'}, 'Location','eastoutside');
title('Self-supervised target, learned TCM readout and fading control scaffold');
