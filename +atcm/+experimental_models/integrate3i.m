function [y,w,s,g,t,pst,layers,noise,firing,QD,Spike] = integrate3i(P,M,U,varargin)
...
%             layers.weighted(ins,:,:) = layers.iweighted(ins,:,:);
        ...
        case {'dmd' 'instantaneous' 'svd'}
            
            switch fmethod
                case 'dmd'
                [Eigenvalues, Eigenvectors, ModeAmplitudes, ModeFrequencies, ...
                    GrowthRates, POD_Mode_Energies] = atcm.fun.dmd(real(yx).*imag(yx), length(Ji), dt);
               % [Eigenvaluesi, Eigenvectorsi, ModeAmplitudesi, ModeFrequenciesi, ...
               %     GrowthRatesi, POD_Mode_Energiesi] = atcm.fun.dmd(imag(yx), length(Ji), dt);
