function [y,w,s,g,t,pst,layers,noise,firing,QD,Spike] = integrate(P,M,U,varargin)
    ...
    if SpecNoise == 1
        for ins = 1:ns
            for ij = 1:length(Ji)
                ...
                y0   = highpass(y0,.1,1/dt,'Steepness',.5)'; 
    ...
    elseif SpecNoise == 0 
        ...
        for ins = 1:ns
            for ij = 1:length(Ji)
                y0 = squeeze(y(ins,ijx(ij),ijy(ij),:))'; % region n, state i,j
                %y0   = highpass(y0',.1,1/dt,'Steepness',.5)'; 
%                 y0 = squeeze(y(ins,ijx(ij),:,:))';
%                 
%                 % remove principal eigenmode from states time series
%                 x = y0;
%                 [u0,s0,v0] = spm_svd(x);
%                 p1 = u0(:,1)*s0(1,1)*v0(:,1)';
%                 %p2 = u0(:,2)*s0(2,2)*v0(:,2)';
...
%                     GrowthRates, POD_Mode_Energies] = atcm.fun.dmd(y0', 1, dt);
%                 y0 = Eigenvectors;
                
                %[Pf,Hz]  = atcm.fun.Afft(y0(burn:end),1/dt,w);
               
                [Pf,Hz]  = atcm.fun.AfftSmooth(y0(burn:end),1/dt,w,50);        
                Pf = Pf';
                Pf = full(Pf)';
                Pf = Pf .* Hz';                                            % off
...
%                         ddx = squeeze(layers.iweighted(inx,:,:));
%                         [Eigenvalues, Eigenvectors, ModeAmplitudes, ModeFrequencies, ...
%                             GrowthRates, POD_Mode_Energies] = atcm.fun.dmd(ddx, 1, 1);
%                         warning on;
%                         Pf(:,inx,iny) = Eigenvectors;
                    end
                else
                    if inx ~= iny
