function [y,w,s,g,t,pst,layers,noise,firing,QD,Spike] = integrate5(P,M,U,varargin)
...
switch IntMethod
    ...
    case 'ode45'
        % integration using matlab built-in ODE45
    ...
    otherwise
        ...
        for i   = 1:length(t) % begin time-stepping loop
            ...
            elseif WithDelays == 45 
                    % RK45 with delayed states
                    % 4-th order Runge-Kutta method.
                    k1 = f(v          ,drive(i)     ,P,M);
                    k2 = f(v+0.5*dt*k1,drive(i)+dt/2,P,M);
                    k3 = f(v+0.5*dt*k2,drive(i)+dt/2,P,M);           
...
% Reshape to model state space outputs (reg*pop*state*time)
%--------------------------------------------------------------------------
[ns,npp,nk] = size(M.x);
y(isnan(y)) = 0;
y(isinf(y)) = 0;
...
IncDCS         = 0; % include a cosine set in the noise model               x       % ON FOR DEXPRO
...
for ins = 1:ns
    ...
    %yx   = highpass(yx',.1,1/dt,'Steepness',.5)'; % removes slow-drits & some 1/f
...
%     dmyx = yx - repmat(mean(yx,2),[1 size(yx,2)]); % demean
%     xcov = cov(dmyx');
%     [u0,s0,v0] = spm_svd(xcov(:,:));
%     ne = findthenearest(cumsum(diag(s0))/sum(diag(s0)),.9);
%     Eigenvectors = u0(:,1:4)'*yx(:,:);
%     %Eigenvectors = highpass(Eigenvectors',1,1/dt)';
%     
%     L23 = u0([1 2 3],1)'*yx([1 2 3],:);
%     L56 = u0([4 5 6],1)'*yx([4 5 6],:);
%     Th  = u0([7 8]  ,1)'*yx([7 8]  ,:);
%     
%     Eigenvectors = [L23 ; L56 ; Th];
   ...
   %idat = fft(dat);
   %idat = symm_orthog(idat')';
   %Eigenvectors  = ifft(dat);
   
   %[U,S,V] = spm_svd(fft(dat));
   
   %Eigenvectors = U'*(dat);
   %[U,T,mu] = pcaecon(fft(dat),nc);
   
   
   %[U,T,mu] = pcaecon( real(hilbert(yx(1:8,:))),nc);
   %[Ui,Ti,mui] = pcaecon( imag(hilbert(yx(1:8,:))),nc);
   ...
   %[Q, D] = ssacom(yx(1:8,:),4);
   %Jy = [.2 .8 0 .2 0 .2 0 .1]*yx(1:8,:);
   %[Q, D] = ssacom(Jy,4);
   
   %Eigenvectors = (U*T);
   
   %Eigenvectors = symm_orthog(Eigenvectors')';
    ...
    %----------------------------------------------------------------------
     for ij = 1:8;%length(Ji)
        ...
        try
            ...
            [Pf,Hz]  = atcm.fun.AfftSmooth(y0(burn:end),1/dt,w,40);          % 60 FOR DEXPRO
            %[Pf,Hz]  = atcm.fun.Afft(y0(burn:end),1/dt,w);Pf=Pf';
        catch
            Hz = w;
            Pf = w'*0;
        end
        ...
        layers.unweighted(ins,ij,:) = abs(( Pf         ) * exp(P.L(ins)));
        layers.weighted  (ins,ij,:) = abs(( Pf * J(ij) ) * exp(P.L(ins)));
        layers.iweighted (ins,ij,:) = abs(( Pf * J(ij) ) * exp(P.L(ins)));
        layers.DMD(ins,ij,:)        = y0;               % retain DMD series
    end
end

% Orthogonalised prediction
% for inx = 1:ns
%     reg = squeeze(layers.iweighted(inx,:,:));
%     for j = 1:8
%         reg(j,:) = reg(j,:) - mean(reg(j,:));
%         reg(j,:) = reg(j,:) ./ std(reg(j,:));
%     end
%     reg = symm_orthog(reg')';
%     layers.iweighted(inx,:,:) = abs(reg);
%     layers.unweighted(inx,:,:) = abs(reg);
%     layers.weighted(inx,:,:) = abs(reg);
% end
