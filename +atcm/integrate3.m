...
%                             
%                             delay(ix) = y(ix,i-abs(Df(ix,iy)));
%                             
%                         end
%                     end
%                     
%                     delterm = ((dxdt - dt*delay)./dt) ;
%                             
%                     if any(delterm)
%                         y(:,i) = (y(:,i)-v) + (dxdt + dt*f(delterm,drive(i),P,M));
%                     else
%                         y(:,i) = y(:,i);
%                     end
%                     
...
%     tt = t./t(end);
%     for i = 1:length(Pks)
%         gx(i,:) = PksAmps(i) * sin(2*pi*Pks(i)*tt);
%     end
%     yx = yx.*sum(gx,1);
...
%                             
%                             for ik = 1:length(Pf2)
%                                 mat(ik,:) = linspace(Pf2(ik),Pf1(ik),4);
%                             end
%                             
%                              YY = spm_vec(M.y);
%                              
...
%                     if ~isfield(M,'I') && isfield(M,'train');
%                         fprintf('Returning glm comps\n');
%                         layers.I{ii} = I;
%                     end
...
%         %Pf(:,ins,ins) = atcm.fun.HighResMeanFilt(Pf(:,ins,ins),1,4);
        ...
        if isfield(M,'EnvLFP') && M.EnvLFP
            ...
            if ~do_movavg
                %tpf = full(atcm.fun.HighResMeanFilt(Pf(:,ins,ins),1,smthk));
...
%                Pf0 = Pf(:,ins,ins);
%                Pf1(:,ins,ins) = full(atcm.fun.HighResMeanFilt(Pf(:,ins,ins),1,smthk));
%                clear opts
%                
%                for ik = 1:length(Pf0)
%                    opts(ik,:) = linspace(Pf0(ik),Pf1(ik),round(smthk/2));
%                end
%                
...
%                Pf(:,ins,ins) = b'*opts';
               ...
               %if isfield(M,'dk')
               %    Pf(:,ins,ins) = Pf(:,ins,ins) + M.dk;
               %end
               
               
               %tpf = Pf(:,ins,ins);
               %k = 6;
               %tpf = mean([ filter(ones(1,k)/k, 1, tpf),flipud(filter(ones(1,k)/k, 1, flipud(tpf)))],2);
               %tpf = flipud(filter(ones(1,k)/k, 1, flipud(tpf)));
               %Pf(:,ins,ins) = tpf;
            
            else
                %Ppf{1} = atcm.fun.moving_average(Pf(:,ins,ins),1);
                %Ppf{2} = atcm.fun.moving_average(Pf(:,ins,ins),2);
                %Ppf{3} = atcm.fun.moving_average(Pf(:,ins,ins),3);
                %Ppf{4} = atcm.fun.moving_average(Pf(:,ins,ins),4);
                ...
                %Ppf{1} = flipud(filter(ones(1,3)/3, 1, flipud(Pf(:,ins,ins))));
                %Ppf{2} = flipud(filter(ones(1,4)/4, 1, flipud(Pf(:,ins,ins))));
                %Ppf{3} = flipud(filter(ones(1,5)/5, 1, flipud(Pf(:,ins,ins))));
                %Ppf{4} = flipud(filter(ones(1,6)/6, 1, flipud(Pf(:,ins,ins))));
