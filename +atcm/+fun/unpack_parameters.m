function [p,v]  = unpack_parameters(DCM,pE)
...
p.C  = spm_unvec( exp(spm_vec(pE.C ))'.*[1 1/64 1/128 1/128] , pE.C  );

% intrisic connection parameters need exp & multiplying by their fixed vals
%--------------------------------------------------------------------------
H     = exp(pE.H);
G     = exp(pE.G);
E(1,:) = [0   0   0   0   0   2   0   2]/1;
E(2,:) = [4   0   0   0   0   0   0   0]/1;
E(3,:) = [4   4   0   0   0   0   0   0]/1; 
E(4,:) = [0   4   0   0   0   0   0   0]/1;
E(5,:) = [0   0   0   4   0   0   0   0]/1;
E(6,:) = [0   0   0   2   0   0   0   1/4]/1; % added RL->TP [Ghodrati 2017]
E(7,:) = [0   0   0   0   0   0   0   2]/1; 
E(8,:) = [0   0   0   0   0   2   0   0]/1;

I(1,:) = [8   0   8   0   0   0   0   0 ];
I(2,:) = [0   16  32  0   0   0   0   0 ];
I(3,:) = [0   0   32  0   0   0   0   0 ];
I(4,:) = [0   0   0   8   12  0   0   0 ];
I(5,:) = [0   0   0   0   16  0   0   0 ];
I(6,:) = [0   0   0   0   8   8   0   0 ];
I(7,:) = [0   0   0   0   0   0   32  0 ];
I(8,:) = [0   0   0   0   0   0   8   32];
...
KN  = exp(-pE.T(:,3))*1000/150;          % excitatory rate constants (NMDA)
...
p.CV   = exp(pE.CV).*  [128 128 64  128 64  128  64  64*2]/1000;  
...
if isfield(pE,'ID')
    ID   = [2 1  .1 2 .2 2 .4 2];
    p.ID = -ID.*exp(pE.ID)/1000;
end

if isfield(pE,'Mh') && any(DCM.M.pC.Mh > 0)
    p.Mh = exp(pE.Mh)*4;
end
if isfield(pE,'Hh') && any(DCM.M.pC.Hh > 0)
    p.Hh = exp(pE.Hh)*4;
end
