...
% Adds option to upsample the ERP data before further processing
...
% optional upsampling factor
try
    upsampleFactor = DCM.options.upsample;  % e.g., 1000 / 128 = 7.8125
catch
    upsampleFactor = 1;  % default: no upsampling
end
...
try
    ...
    if isfield(DCM.options,'Bdcm');
        ...
        [~, B1]     = min(abs(DCM.xY.Time - B1));
        [~, B2]     = min(abs(DCM.xY.Time - B2));
    end
    It          = (T1:DT:T2)';
    try Bt      = (B1:DT:B2)'; end
    Ns          = length(It);
    DCM.xY.pst  = DCM.xY.Time(It);
    DCM.xY.dt   = DT/D.fsample;
    DCM.xY.It   = It;
catch
...
for i = 1:length(trial)
    ...
    if isfield(DCM.options,'TrlPrcnt'); 
        nT  = length(c)/100;
        nT  = round(nT*DCM.options.TrlPrcnt);
        NNc = nT(1):1:nT(2);
        if  NNc(1) == 0; NNc = NNc + 1; end
        NNc  = c(NNc);
    ...
    if ERP
        ...
        Y = Y/Nt;
        if upsampleFactor > 1
            Y = resample(Y, round(upsampleFactor), 1);
            DCM.xY.pst = linspace(DCM.xY.pst(1), DCM.xY.pst(end), size(Y,1))';
            DCM.xY.dt = (DCM.xY.pst(2) - DCM.xY.pst(1))/1000;

            Ns = length(DCM.xY.pst);
        ...
        if upsampleFactor > 1
            for j = 1:Nt
                Y(:,:,j) = resample(Y(:,:,j), round(upsampleFactor), 1);
            end
            DCM.xY.pst = linspace(DCM.xY.pst(1), DCM.xY.pst(end), size(Y,1))';
            DCM.xY.dt = (DCM.xY.pst(2) - DCM.xY.pst(1))/1000;
        end
