function D = convert_ve_to_spmlfp(ftdata, filename)
...
% Vladimir Litvak
% Krish Singh, Cardiff, changed default-type to lfp
% $Id: spm_eeg_ft2spm.m 4038 2010-08-10 10:39:00Z vladimir $
% Alex added ability to include trial name in ftdata.conditionname
%
%
isTF = 0;
...
% Alex allow specification of condition name
f = fieldnames(ftdata);
if any(strcmp(f,'conditionname'));
    if length(ftdata.conditionname) == Ntrials;
        for i = 1:Ntrials
            D.trials(i).label = ftdata.conditionname{i};
        end
    else
        D.trials = repmat(struct('label',{ftdata.conditionname}),1,Ntrials);
    end
else
...
% fprintf(1,'Setting channel types to LFP before saving\n');
% D.channels.type='LFP';
% D
% D.channels
% D.channels.type
