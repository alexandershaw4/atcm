function [f,J,Q] = spm_fx_thal(x,u,P,M)
% state equations for canonical 2-pop thal
%
...
CV   = exp(P.CV).*[128 128]/1000;  % membrane capacitance
...
for i = 1;%:ns
        ...
        % Voltage
        %==================================================================
          f(i,1:2,1) =    (GL*(VL - x(i,1:2,1))+...
                         x(i,1:2,2).*(VE - x(i,1:2,1))+...
                         x(i,1:2,3).*(VI - x(i,1:2,1))+...
                         x(i,1:2,4).*(VN - x(i,1:2,1)).*mg_switch(x(i,1:2,1)))./CV;
        
        % Conductance
        %==================================================================
        f(i,1:2,2) = (E' - x(i,1:2,2)).*KE(i,:);
        f(i,1:2,3) = (I' - x(i,1:2,3)).*KI(i,:);
        f(i,1:2,4) = (ENMDA' - x(i,1:2,4))*KNMDA(i,:) ;
end
