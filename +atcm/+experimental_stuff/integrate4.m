function [y,w,s,g,t,pst,layers,noise,firing,QD,Spike] = integrate4(P,M,U,varargin)
...
% and with different trial types. This version (atcm.integrate4) uses
...
% (voltages, currents). Finally, DMD & weighting, & an fft of  the resultant 
...
%
% In this version (intgerate4), the DMD routine returns a single output -
% so the system is single-output and somewhat linearised. See
% integrate3.m, where the DMD routine reduces to multi-output. That
% converges slower.
%
...
for i = 1:size(P.c,2)
    %Gs(:,i) = exp(P.c(1,i) - 2)+w.^(-exp(P.c(2,1)));
    Gs(:,i) = exp(P.c(1,i) )+w.^(-exp(P.c(2,i)));
...
for ins = 1:ns
    ...
    %yx = yx([1 2 4 6],:);    
     ...
     %yx(1,:) = yx(1,:) * .2; % l4   stellates
     %yx(2,:) = yx(2,:) * .8; % l2/3 pyramids
     %yx(3,:) = yx(3,:) * .2; % l5   pyramids
     %yx(4,:) = yx(4,:) * .2; % l6   pyramids
    ...
    % use dynamic mode decomposition (DMD) to reduce the membrane oscillatory
    ...
    [Eigenvalues, Eigenvectors, ModeAmplitudes, ModeFrequencies, ...
        GrowthRates, POD_Mode_Energies] = atcm.fun.dmd(yx', 1, dt);
    Eigenvectors = Eigenvectors*yx;
    
    %[u0,s0,v0] = spm_svd(hilbert(yx));
