function [f,J,Q,D,DV] = tcm_nomh_red(x,u,P,M,m)
%#codegen
...
GEa(1,:) = [0   0   0   0   0   0   4]/1;
GEa(2,:) = [4   0   0   0   0   0   0]/1;
GEa(3,:) = [4   4   0   0   0   0   0]/1; 
GEa(4,:) = [0   4   0   0   0   0   2]/1;
GEa(5,:) = [0   0   0   4   0   0   0]/1;
GEa(6,:) = [0   0   0   0   0   0   2]/1; 
GEa(7,:) = [0   0   0   4   0   0   0]/1;
...
GIa(1,:) = [8   0   2   0   0   0   0 ];% ss
GIa(2,:) = [0   16  16  0   0   0   0 ];% sp
GIa(3,:) = [0   0   32  0   0   0   0 ];% si
GIa(4,:) = [0   0   0   8   8   0   0 ];% dp
GIa(5,:) = [0   0   0   0   16  0   0 ];% di
GIa(6,:) = [0   0   0   0   0   32  0 ];% rt
GIa(7,:) = [0   0   0   0   0   8   32];% rl
...
% membrane capacitances {ss  sp  ii  dp  di     rt  rl}
%--------------------------------------------------------------------------
CV   = exp(P.CV).*      [128 32  32  128 64    256 64]/1000;  
...
%a(:,3)  = A{3}*m(:,5);                      % thalamic projection pyramids
a(:,4)  = A{4}*m(:,6);                      % reticular AMPA
a(:,5)  = A{5}*m(:,7);                      % relay AMPA
an(:,1) = AN{1}*m(:,2);                     % forward afference  NMDA
an(:,2) = AN{2}*m(:,4);                     % backward afference NMDA
%an(:,3) = AN{3}*m(:,6);                     % thalamic projection pyramids
an(:,4) = AN{4}*m(:,6);                     % reticular NMDA
an(:,5) = AN{5}*m(:,7);                     % relay NMDA
...
for i = 1:ns
        ...
        %------------------------------------------------------------------
        input_cell        = [7 1 2 4];
        E(input_cell)     = E(input_cell)         +U';
        ENMDA(input_cell) = ENMDA(input_cell)     +U';
