function [y,w,s,g,t,pst,layers,noise,firing,QD,Spike] = integrate_red(P,M,U,varargin)
...
%--------------------------------------------------------------------------
f   = spm_funcheck(M.f); 
x   = atcm.solvefixedpoint_red(P,M);
%x        = x*0;
...
for  c = 1:size(U.X,1)
    ...
    %----------------------------------------------------------------------
    [y{c},w,s{c},g{c},t{c},layers{c},noise{c},firing{c},QD{c},Spike{c}] = dodxdt(pst,f,v,Q,M,dt,w,drive);
...
if WithDelays == 2 || WithDelays == 5 || strcmp(IntMethod,'ode45')
...
for ins = 1:ns
    ...
    %ts0       = (~~J'*xseries)';
    ts0 = (sparse(1:7,1,1,7*5,1)'*xseries)';
...
for ins = 1:ns
    for ij = 1:length(Ji)
        ...
        % Smoothing
        %------------------------------------------------------------------
        %Pf       = smooth(Hz,Pf,0.15,'loess');    %now off
        %Pf       = smooth(Hz,Pf,0.20,'loess');    %now off
        %Pf       = smooth(Hz,Pf,0.20,'loess');   
        Pf       = full(Pf);
        J        = full(J);
        
        % spectral smoothing with kernel increasing with frequency
        %Pf0 = PfSmooth(Pf);
