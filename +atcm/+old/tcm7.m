function [f,J,Q,D,IntrCon,rate] = tcm7(x,u,P,M)
...
% %           ss  sp  si  dp  di     rt  rl   
% GEa(1,:) = [0   0   0   0   0      0   4]/10;
% GEa(2,:) = [4   0   0   0   0      0   0]/10;
% GEa(3,:) = [4   4   0   0   0      0   0]/10;
% GEa(4,:) = [0   4   0   0   0      0   0]/10;
% GEa(5,:) = [0   0   0   4   0      0   0]/10;
% GEa(6,:) = [0   0   0   0   0      0   2]/10;
% GEa(7,:) = [0   0   0   0   0      0   0]/10;
% 
% % This is the Traub et al 2004 TC model excitatory connectivity pattern
% %           ss  sp  si  dp  di    rt  rl   
% GEa(1,:) = [0   4   0   4   0      0   4]/10;
% GEa(2,:) = [4   0   0   4   0      0   4]/10;
% GEa(3,:) = [4   4   0   4   0      0   4]/10;
% GEa(4,:) = [4   4   0   0   0      0   4]/10;
% GEa(5,:) = [4   4   0   4   0      0   4]/10;
% GEa(6,:) = [0   0   0   0   0      0   0]/10;
% GEa(7,:) = [0   0   0   0   0      0   0]/10;

% This is the CMC13 with thalamic cells & L5 interneurons
%           ss  sp  si  dp       rt  rl   
GEa(1,:) = [0   0   0   0        0   2]/1; % ss
GEa(2,:) = [8   0   0   0        0   0]/1; % sp
GEa(3,:) = [4   4   0   4        0   0]/1; % si
GEa(4,:) = [0   4   0   0        0   0]/1; % dp
GEa(5,:) = [0   0   0   4        0   4]/1; % rt
GEa(6,:) = [0   0   0   0        0   0]/1; % rl

GEa = GEa .* ~eye(np);
GEa = GEa + eye(np);

% % Append self-excitation parameters {excitatory cells only} (AMPA)
% %--------------------------------------------------------------------------
for i = 1:ns
    sG(:,i) = exp(P.G(i,:)) .* [4 4 0 4 0 4];
end
...
% % M- channel connections (np x np) {L6 & Thal Relay cells only}
% %--------------------------------------------------------------------------
% GIm  = eye(8)*4/10;
% GIm  = sparse([6 8],[6 8],4/10,8,8);
% Mh   = diag(exp(P.Mh));
%  
% % H-channel conductances {L6 & Thal Relay cells only}
% %--------------------------------------------------------------------------
...
%CR = exp(-P.TV).* (1000./[20 2 4 40 10 50 20 80]); % cell conduction velocity
CR = exp(-P.TV).*1000./[10 2 1 20 40 60];
...
% membrane capacitances {ss  sp  ii  dp   rt  rl}
%--------------------------------------------------------------------------
CV   = exp(P.CV).*[128 128 256 32 128 128]/1000;  
CV   = exp(P.CV).*[32 64 256 32 128 32]/1000;  
CV   = exp(P.CV).*[32 64 64 32 64 32]/1000;  
...
a(:,4)  = A{4}*m(:,5);                      % reticular AMPA
a(:,5)  = A{5}*m(:,6);                      % relay AMPA
an(:,1) = AN{1}*m(:,2);                     % forward afference  NMDA
an(:,2) = AN{2}*m(:,4);                     % backward afference NMDA
an(:,4) = AN{4}*m(:,5);                     % reticular NMDA
an(:,5) = AN{5}*m(:,6);                     % relay NMDA
...
%--------------------------------------------------------------------------
Tc = zeros(np,np);
%Tc([5 6],[1:4])       = 60; % cortex --> thalamus     = 60 ms
%Tc(1:4,[5 6])         = 20; % thalamus --> cortex     = 20 ms
Tc = -Tc / 1000;
