...
switch IntMethod
    ...
    case 'linearise'
        ...
        % note: M.x should be at a fixed point
...
%             warning off;
             Ppf = ( squeeze(Pfm)' );
%             [eval, evec] = atcm.fun.dmd(Ppf', 5, dw);
%             Ppf = evec*Ppf;
%             warning on;
%              warning off;
            for ip = 1:size(Ppf,1)
                %m0 = fit(w(:),real(Ppf(ip,:)'),'Gauss4');
                %Ppf(ip,:) = m0(w);
                Ppf(ip,:) = atcm.fun.moving_average(Ppf(ip,:),4);
            end
            
            Ppf = mean(Ppf,1);
%              warning on;
            %b   = atcm.fun.lsqnonneg(real(Ppf)',real(M.y{ci}(:,ins,ins)));
            %b   = pinv(Ppf*Ppf')*Ppf*squeeze(M.y{ci}(:,ins,ins));
            %Ppf = spm_vec(b'*Ppf);

            % Retain principal eigenmode over (Gaussian) windows
            %[u,s,v] = svd(Ppf');n = 1;
            ...
            %-------------------------------------------------------------- 
            tmp = Pf; % g = fittype( 'exp(a).*x.^(-exp(b))' );
            c   = atcm.fun.c_oof(w(:),tmp,'exp2');% constrained exponent
            m   = fit(w(:),real(tmp-c),'Gauss6'); % gmm on residual
            r   = Pf - (m(w) + c);
            Pf  = (c/4) + m(w) ;%+ (r/4);
