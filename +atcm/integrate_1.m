...
% exponential (atcm.fun.c_oof) and GMM. 
% 
% Other options:
% -------------------------------------
...
switch InputType
    ...
    case 4
        ...
        %------------------------------------------------------------------
        mu1   = .5*exp(P.R(1));                % mean amplitude
        mu2   = 4*exp(P.R(2));
...
if isfield(P,'ID')
    ...
    if npp == 8 
        ...
        del = exp(P.ID).*[4 1/5 1 1/2 1/2 4 2 20]/2.4;
        %del = exp(P.ID).*[1 1/5 1 1 1 1 1 20]/2.4;
        %del = exp(P.ID).*[1 1/4 1 1 1 4 2 20]/2.4;
...
% using a delay operator (Q) following Q ~ exp(dt*D*(del*dfdx)./N - I)
%--------------------------------------------------------------------------
if isfield(P,'ID')
    p       = abs(eigs(del.*dfdx,1,'SR',OPT));
    N       = ceil(max(1,dt*p*2));
    n       = spm_length(M.x);
    dfdx    = dfdx - speye(n,n)*exp(-16);
...
switch IntMethod
    ...
    case 'ode45'
        ...
        ode = @(t,v,P,M,f) spm_vec( (del)*f(spm_unvec(v,M.x),drive,P,M) );
...
% Gaussian mixture model - estimate & scale [parameterised] system noise
% per mode / region
%--------------------------------------------------------------------------
for ins = 1:ns

    % Gaussian mixture model - estimate & scale system noise
    %----------------------------------------------------------------------
    fullPf = Pf(:,ins,ins);
    [~,pk] = atcm.fun.maxpoints((Pf(:,ins,ins)),50);
    wint   = 1:length(w);
    %Pf(:,ins,ins) = atcm.fun.makef(wint,wint(pk)-1,Pf(pk,ins,ins),ones(length(find(pk)),1)*3,'cauchy');
    Pf(:,ins,ins) = atcm.fun.findbestdist(wint,wint(pk)-1,Pf(pk,ins,ins),ones(length(find(pk)),1)*3,M.y{1});
    ...
    addnoise=0;
    if addnoise
        % Multiply in the {dct} neuronal fluctuations
        for i = 1:length(Hz)
