function [f,J,Q,D] = tc_ketdep6(x,u,P,M,m)
...
% %--------------------------------------------------------------------------
%       SP  SP  DP  DP 
SA   = [1   0   0   0;   %  SS    % added TP->SP
        0   0   1   0;   %  SP
        0   0   0   1;   %  SI
        0   1   0   0;   %  DP
        0   0   0   0;   %  DI
        0   0   0   0]/8;%  rc % 0 in ket study
...
GEa(4,:) = [0   4   0   0   0   0 ]/1;
GEa(5,:) = [0   0   0   4   0   0 ]/1;
GEa(6,:) = [0   0   0   2   0   0 ]/1; % added RL->TP [Ghodrati 2017]
...
GEa(4,:) = [0   6   0   0   0   2 ]/1;
GEa(5,:) = [0   0   0   4   0   2 ]/1;
GEa(6,:) = [0   0   0   6   0   0 ]/1; % added RL->TP [Ghodrati 2017]
...
GIa(1,:) = [16  0   8   0   0   0  ];
GIa(2,:) = [0   32  16  0   0   0  ]; %spsp was 16
GIa(3,:) = [0   0   32  0   0   0  ];
GIa(4,:) = [0   0   0   8   12  0  ];
GIa(5,:) = [0   0   32  0   16  0  ];
GIa(6,:) = [0   0   0   0   32  8  ];
...
if IncludeMH
    ...
    GIm  = sparse([6],[6],4,6,6);
    Mh   = diag(exp(P.Mh));

    GIh      = full(sparse([6],[6],4   ,6,6)); % 1/4
    Hh       = exp(P.Hh);
...
%--------------------------------------------------------------------------
CV   = exp(P.CV).*      [128 128 128  128 64  128]/1000;  
...
a(:,2)  = A{2}*m(:,2);                      % forward afference  AMPA - SP->DP
a(:,3)  = A{3}*m(:,4);                      % backward afference AMPA - DP->SP
a(:,4)  = A{4}*m(:,4);                      % backward afference AMPA - DP->II

an(:,1)  = AN{1}*m(:,2);                      % forward afference  AMPA - SP->SS
an(:,2)  = AN{2}*m(:,2);                      % forward afference  AMPA - SP->DP
an(:,3)  = AN{3}*m(:,4);                      % backward afference AMPA - DP->SP
an(:,4)  = AN{4}*m(:,4);                      % backward afference AMPA - DP->II
...
if ~isfield(P,'delays')
    ...
    D = d(1)*Ds  ;       %+ Dself;% Complete delay matrix
end
