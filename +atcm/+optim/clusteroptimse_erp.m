function [X0,X,cm,F,j,others] = clusteroptimse_erp(P,DCM,method,nc)
global DD
...
for i = 1:nr
    ...
    switch method
        ...
        c   = constr(Px');
        DD.doplot = 0;
        [X,F(i)]  = atcm.optim.abcAS(@optimi,Px,c);
        ...
        lu          = [(Px-(dx*4))';(Px+(dx*4))']; % Seach space
        ...
        %[X,F(i)]  = agradoptimi(@optimi,Px(:),c,0);
        ...
        %[X,F(i)]  = agradoptimi(@fakeDM,Px(:),c,DCM.xY.y);
        %[X,F(i)]  = aoptim_edge(@fakeDM,Px(:),c,DCM.xY.y);
        
        [X,F(i)]  = AO(@fakeDM,Px(:),c,DCM.xY.y,5,12*4,[],1e-3,0.000001);
        %[X,F(i)]  = AO(@fakeDM,Px(:),c,DCM.xY.y,128,2);
...
%             UB  = (Px+(c'*4));
        ...
        case {'PR','minimise','hinton'};
            ...
            [X, F, ix] = PR_minimize(Px, @optimi, 128);
...
% %y  = spm_vec(y);
% 
% % observer projection, residuals, precision
% %--------------------------------------------
% L = feval( DD.M.G , PP , DD.M); % g(x)
% R = DD.M.R;
% 
% for i = 1:length(y)
%     y{i} = R*y{i}*L';
...
if DD.doplot
    for i = 1:length(y)
        ...
        pst = DD.xY.pst;
        plot(pst,DD.xY.y{i},':',pst,y{i}); drawnow;
    end
end
...
% if isfield(DD.xY,'Q') && any(DD.xY.Q{1}(:))
%     Q = DD.xY.Q{1};
% end
...
% DM.M.nograph = 1;
% DM.M.pC      = DD.M.pC;
% DM.M.IS      = @ISerp;    % see this intermediary subfun below
% E            = atcm.optim.fe0(DM.M,DM.xU,DM.xY,PP,Gmax);
% E            = -real(E);
% 
% if E < Gmax
...
%             hold off;
        pst = DD.xY.pst;
        for i = 1:length(yy)
            subplot(1,length(yy),i);
            plot(pst,Y{i},':','linewidth',2); hold on;
            plot(pst,yy{i},'linewidth',2); hold off;
            drawnow;
        end
...
% { dx = f(x,u,P,M)
%
% play nicely with the free energy calculation in atcm.optim.fe0
% by lumping the g and f into 1 function
%


% motion - f(x)
yy = feval(DD.M.IS,PP,M,xU);

% evaluate oberver func - g(x)
L = feval( DD.M.G , PP , DD.M);
R = DD.M.R;
...
c = clusterdata(full(real(CV)),'linkage','ward','maxclust',nc);
...
% Compute the Jacobian matrix using variable step-size
n  = 0;
fx = objectf(IS,spm_unvec(Ep,P),M,U,Y);
warning off ;
...
global DD
% An objective function for models of the form
%
% { y  = g(x,..)
% { x  = f(x,..)
%
...
%y = spm_vec( spm_cat(IS(P,M,U)) );
%X = spm_vec( spm_cat(Y.y      ) );

y = ( (IS(P,M,U)) );            % motion
L = feval( DD.M.G , P , DD.M);  % observer
R = DD.M.R;                     % residuals
