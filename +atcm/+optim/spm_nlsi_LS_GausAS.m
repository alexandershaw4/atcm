function [Ep,qC,qh,F] = spm_nlsi_LS_GausAS(M,U,Y)
% - ALEX LONG VERSION: MUCH MORE SAMPLING!
%
...
% Alex: allow specification of number of samples per iteration
ngibbs = 64;
...
%==========================================================================
Gmax  = -Inf;
for k = 1:(64*2) % alex: bump from 64 to 128
    ...
    for i = 1:ngibbs
        ...
        % Alex added sampling routines:
        P(:,i) = qE + spm_samp_gauss(pE, full(qS), 1)'; % gaussian sampling
        
        % MC Parsimonious Adaptive Rejection Sampling - Nakagami sampling
        %P(:,i) = qE + qS*PARS_nakagami(np-1,.8)';           
        ...
        % [Alex]: disp(i) is too verbose, try
        %---------------------------------------
        if i > 1; fprintf(repmat('\b',size(str))); end
        str = sprintf('%d/%d',i,ngibbs);
        fprintf(str);
        if i == ngibbs; fprintf('\n'); end
