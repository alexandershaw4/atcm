function [Ep,Cp,Eh,F,L,dFdp,dFdpp] = spm_nlsi_GN_as(M,U,Y)
...
if ~M.nograph
    ...
    Fsi = figure('position',[983          40        1169         945]);
end
...
for k = 1:M.Nmax
    ...
    if exist('Fsi', 'var')
        ...
        if length(x) == 1 && length(e) == length(Y.Hz)
            x = Y.Hz;
        end
        
        % plot real or complex predictions
        %------------------------------------------------------------------
        tstr = sprintf('%s: %i','data and prediction: E-Step',k);
        if isreal(spm_vec(y))
            ...
            try
                warning off;
                [~,~,s0,~,~,pst0,l0] = feval(IS,Ep,M,U);
                plot(x,real(squeeze(l0{1}.weighted(1,:,:))),'linewidth',2);
                xlabel('Frequency (Hz)');grid on;ylabel('PSD');
                title('(contributing) states spectra');
                
            
                %subplot(413),
                subplot(2,2,3)
                s0 = real(squeeze(s0{1}(1,:,1,:)));
                plot(pst0,s0);grid on;xlabel('time');ylabel('membrane (mV)');
                title('population membrane potentials');
                warning on;
            end
